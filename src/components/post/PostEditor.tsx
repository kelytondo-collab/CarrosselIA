...
import { generatePostCopy, generatePostFormat, analyzeReference, generateFromReference } from '../../services/geminiService'
...
export default function PostEditor() {
  ...
  const [mode, setMode] = useState<'luminae' | 'create' | 'clone'>('create')
  ...
  const handleClone = async () => {
    if (!apiKey) { toast.error('Configure sua chave Gemini nas Configuracoes'); return }
    if (!referenceImage) { toast.error('Envie uma imagem de referencia'); return }
    if (!cloneTheme.trim()) { toast.error('Informe o tema para o seu post'); return }

    setAnalyzing(true)
    const toastId = toast.loading('Analisando referencia...')

    try {
      toast.loading('Extraindo cores, layout e tipografia...', { id: toastId })
      const analysis = await analyzeReference(referenceImage, niche, tone)

      toast.loading('Gerando post inspirado...', { id: toastId })
      const postData = await generateFromReference(analysis, cloneTheme, niche, tone, defaultProfile?.voiceBlueprint)

      const project = createSimpleProject(`Clone: ${cloneTheme}`, cloneTheme, 'post')
      updateProjectPost(project.id, postData)
      refreshProjects()

      setCurrentProject({ ...project, current_post_data: postData })
      setView('post-preview' as any)
      toast.success('Post clonado!', { id: toastId })
    } catch (err: unknown) {
      toast.error(err instanceof Error ? err.message : 'Erro', { id: toastId })
    } finally {
      setAnalyzing(false)
        ...
        <div className="space-y-5">
          <div className="bg-white dark:bg-gray-800/50 rounded-2xl border border-gray-200 dark:border-gray-700 p-5 space-y-4">
            <div className="rounded-xl p-3 border border-amber-200 dark:border-amber-800/40 bg-amber-50 dark:bg-amber-900/10 text-xs text-amber-700 dark:text-amber-300 leading-relaxed">
              <span className="font-bold">Como funciona:</span> Envie um screenshot de um post/anuncio que voce gostou. A IA analisa cores, layout e estilo, e gera um post SIMILAR com a SUA identidade visual.
            </div>

            <div>
              <label className={labelCls}>Screenshot de referencia *</label>
              <input ref={fileRef} type="file" accept="image/*" className="hidden" onChange={handleReferenceUpload} />
              {referenceImage ? (
                <div className="flex items-start gap-3">
                  <img src={referenceImage} alt="Ref" className="w-32 h-32 object-cover rounded-xl border border-gray-200 dark:border-gray-700" />
                  <div className="flex-1">
                    <p className="text-sm text-gray-700 dark:text-gray-300 font-medium">Referencia carregada</p>
                    <p className="text-xs text-gray-400 mt-0.5">A IA vai analisar cores, layout e estilo</p>
                ...
                <button onClick={() => fileRef.current?.click()} className="w-full py-8 border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl text-gray-400 hover:border-violet-400 hover:text-violet-500 transition-all flex flex-col items-center gap-2">
                  <Upload size={24} />
                  <span className="text-sm">Enviar screenshot</span>
                </button>
              )}
            </div>

            <div>
              <label className={labelCls}>Tema do SEU post *</label>
          ...
          <button onClick={handleClone} disabled={analyzing} className="w-full py-4 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 disabled:opacity-50 text-white font-bold rounded-2xl text-base transition-all shadow-lg flex items-center justify-center gap-3">
            {analyzing ? <><Loader2 size={20} className="animate-spin" /> Analisando...</> : <><Image size={20} /> Clonar e Adaptar</>}
          </button>
        </div>
      )}
    </div>
